...
function createAccordion(id,controls,target,open,heading,jsonData,index){
    //    console.log(id,controls,target,open,heading,url);
        console.log(index)
        ...
        accoItem.innerHTML = `
            ...
            <div id="${controls}" class="accordion-collapse collapse ${index===0?'show':''}" aria-labelledby="${heading}" data-bs-parent="#accordionExample">
...
const init = async() =>{
    magazines.forEach(async(element,index)=>{
        ...
        let jsonFile = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURI(element)}`);
        ...
        let accordion = createAccordion(id,controls,target,open,heading,jsonData,index);
        ...
        // let accBody = document.getElementsByClassName("carousel-inner");
        // let carousel = createCarousal(jsonData,index);
        // accBody.append(carousel);
